...
static async getDailyWorkHoursInMinutes(profileId) {
  ...
  static async findAllWithFilters(filters = {}) {
    let sql = `
      SELECT p.nome, s.nome as setor_nome
      FROM perfis p
      JOIN usuarios u ON p.usuario_id = u.id
      LEFT JOIN setores s ON p.setor_id = s.id
  ...
  static async update(id, profileData) {
      ...
      if (profileData.nome) { fields.push(`nome = $${paramIndex++}`); params.push(profileData.nome); }
      if (profileData.gerente !== undefined) { fields.push(`gerente = $${paramIndex++}`); params.push(profileData.gerente); }
      if (profileData.ch_primeira) { fields.push(`ch_primeira = $${paramIndex++}`); params.push(profileData.ch_primeira); }
      if (profileData.ch_segunda) { fields.push(`ch_segunda = $${paramIndex++}`); params.push(profileData.ch_segunda); }
      if (profileData.setor_id) { fields.push(`setor_id = $${paramIndex++}`); params.push(profileData.setor_id); }
      if (profileData.endereco !== undefined) { fields.push(`endereco = $${paramIndex++}`); params.push(profileData.endereco); }
      if (profileData.foto_url) { fields.push(`foto_url = $${paramIndex++}`); params.push(profileData.foto_url); }

      if (profileData.data_nascimento !== undefined) { fields.push(`data_nascimento = $${paramIndex++}`); params.push(profileData.data_nascimento || null); }
      if (profileData.sexo !== undefined) { fields.push(`sexo = $${paramIndex++}`); params.push(profileData.sexo); }
      if (profileData.cpf !== undefined) { fields.push(`cpf = $${paramIndex++}`); params.push(profileData.cpf); }
      if (profileData.telefone !== undefined) { fields.push(`telefone = $${paramIndex++}`); params.push(profileData.telefone); }
      if (profileData.linkedin !== undefined) { fields.push(`linkedin = $${paramIndex++}`); params.push(profileData.linkedin); }
      if (profileData.cep !== undefined) { fields.push(`cep = $${paramIndex++}`); params.push(profileData.cep); }
      if (profileData.logradouro !== undefined) { fields.push(`logradouro = $${paramIndex++}`); params.push(profileData.logradouro); }
      if (profileData.bairro !== undefined) { fields.push(`bairro = $${paramIndex++}`); params.push(profileData.bairro); }
      if (profileData.cidade !== undefined) { fields.push(`cidade = $${paramIndex++}`); params.push(profileData.cidade); }
      if (profileData.estado !== undefined) { fields.push(`estado = $${paramIndex++}`); params.push(profileData.estado); }
      if (profileData.funcao !== undefined) { fields.push(`funcao = $${paramIndex++}`); params.push(profileData.funcao); }
  ...
  static async create(profileData) {
    const { nome, gerente, ch_primeira, ch_segunda, setor_id, usuario_id } = profileData;
    const sql = `
      INSERT INTO perfis (nome, gerente, ch_primeira, ch_segunda, setor_id, usuario_id)
      VALUES ($1, $2, $3, $4, $5, $6)
      RETURNING *
    `;
    const result = await db.query(sql, [nome, gerente, ch_primeira, ch_segunda, setor_id, usuario_id]);
    return result.rows[0];
  }
