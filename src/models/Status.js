...
class Status {
  ...
  static async create(statusData) {
    ...
    const result = await db.run(
      'INSERT INTO status (nome, analise, autorizado) VALUES ($1, $2, $3) RETURNING id',
  ...
  static async findById(id) {
    return await db.get('SELECT * FROM status WHERE id = $1', [id]);
  }

  // Busca um status pelo nome
  static async findByName(nome) {
    return await db.get('SELECT * FROM status WHERE nome = $1', [nome]);
  }

  // Lista todos os status
  static async findAll() {
    return await db.all('SELECT * FROM status ORDER BY nome');
  }
  ...
  static async update(id, statusData) {
    ...
    await db.run(
      'UPDATE status SET nome = $1, analise = $2, autorizado = $3, updated_at = CURRENT_TIMESTAMP WHERE id = $4',
  ...
  static async delete(id) {
    ...
    const result = await db.run('DELETE FROM status WHERE id = $1', [id]);
    return result.changes > 0;
  }

  // Status padrão
  static async getPendingStatus() {
    return await db.get('SELECT * FROM status WHERE analise = TRUE LIMIT 1');
  }

  static async getApprovedStatus() {
    return await db.get('SELECT * FROM status WHERE autorizado = TRUE AND analise = FALSE LIMIT 1');
  }

  static async getRejectedStatus() {
    return await db.get('SELECT * FROM status WHERE autorizado = FALSE AND analise = FALSE LIMIT 1');
  }
