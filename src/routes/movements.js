...
router.post('/', isApiAuthenticated, [ // <-- ALTERADO AQUI para isApiAuthenticated
    ...
    try {
        ...
        if (!errors.isEmpty()) {
            // Retorna o erro de validação em JSON
            return res.status(400).json({ message: 'Dados inválidos', details: errors.array() });
        }

        const { data_movimentacao, hora_inicial, hora_final, hora_total, motivo, entrada, forma_pagamento_id } = req.body;

        if (!req.userProfile) {
            return res.status(403).json({ message: 'Usuário não possui perfil cadastrado' });
        }
        const colaborador_id = req.userProfile.id;

        const pendingStatus = await Status.findByName('Pendente'); // Busca o status 'Pendente'
        if (!pendingStatus) {
            return res.status(500).json({ message: 'Status padrão "Pendente" não encontrado no sistema.' });
        }
        ...
        // 2. Criar o log da movimentação
        await MovementLog.logMovementCreation(newMovement.id, req.userProfile.id);

        // 3. Enviar resposta de sucesso
        res.status(201).json({
            success: true,
            message: 'Movimentação criada e enviada para aprovação com sucesso!',
            movement: newMovement
        });

    } catch (error) {
        console.error('Erro ao criar movimentação:', error);
        // Garante que a resposta de erro seja em JSON
        res.status(500).json({ message: 'Erro interno do servidor ao criar a movimentação.' });
    }
});
...
router.post('/', isApiAuthenticated, [
    body('data_movimentacao').isISO8601().withMessage('Data da movimentação inválida'),
    body('entrada').isBoolean().withMessage('Entrada deve ser um valor booleano')
], async (req, res) => {
    try {
        ...
        const { data_movimentacao, hora_total, motivo, entrada } = req.body;
        const colaborador_id = req.userProfile.id;
        ...
        const movementData = { data_movimentacao, hora_total, motivo, entrada, status_id: pendingStatus.id, colaborador_id };
        const newMovement = await Movement.create(movementData);
        await MovementLog.logMovementCreation(newMovement.id, colaborador_id);

        res.status(201).json({ success: true, message: 'Movimentação enviada para aprovação!' });
    } catch (error) {
        console.error('Erro ao criar movimentação:', error);
...
router.post('/solicitar-folga', isApiAuthenticated, async (req, res) => { 
    try {
        ...
        const dailyWorkMinutes = await Profile.getDailyWorkHoursInMinutes(profileId);

        // 2. Validar se o colaborador tem saldo positivo
        if (balance.total_minutes <= 0) {
            return res.status(400).json({ success: false, message: "Você não possui saldo de horas positivo para solicitar uma folga." });
        }
        ...
        if (tipo_folga === 'integral') {
            ...
            if (balance.total_minutes < dailyWorkMinutes) {
                return res.status(400).json({ success: false, message: "Saldo de horas insuficiente para uma folga integral." });
            }
            const hours = Math.floor(dailyWorkMinutes / 60).toString().padStart(2, '0');
            ...
            if (minutosASeremDebitado <= 0) {
                 return res.status(400).json({ success: false, message: "A quantidade de horas parciais deve ser maior que zero." });
            }
            if (balance.total_minutes < minutosASeremDebitado) {
                return res.status(400).json({ success: false, message: "Saldo de horas insuficiente para a quantidade de horas solicitada." });
            }
        ...
        res.status(201).json({ success: true, message: 'Solicitação de folga enviada com sucesso!' });
